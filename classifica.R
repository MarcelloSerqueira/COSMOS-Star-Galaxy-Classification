#
# FUN√á√ïES DA CLASSIFICA√á√ÉO DE ESTRELAS E GAL√ÅXIAS
#
#
# REMOCAO DE OUTLIERS
#
...
rn.deep <- function(x.train, x.test, x.val = NULL, clabel, tamanho = c(200,200), ...) 
{
  require(h2o)
  
  #TransformaÁ„o do dataset de treino
  predictors_name_train  = setdiff(colnames(x.train), clabel)
  predictors_train = as.data.frame(x.train[,predictors_name_train])
  predictand = x.train[, clabel]
  alvo <- as.factor(predictand)
  predictors_train[ncol(predictors_train)+1] = alvo
  colnames(predictors_train)[ncol(predictors_train)] = clabel
  x.train = predictors_train
  
  #TransformaÁ„o do dataset de teste
  predictors_name_test  = setdiff(colnames(x.test),clabel)
  predictors_test = as.data.frame(x.test[,predictors_name_test])
  predictand = x.test[, clabel]
  alvo <- as.factor(predictand)
  predictors_test[ncol(predictors_test)+1] = alvo
  colnames(predictors_test)[ncol(predictors_test)] = clabel
  x.test = predictors_test
  
  #TransformaÁ„o do dataset de validaÁ„o
  if(is.null(x.val) == FALSE){
    predictors_name_val  = setdiff(colnames(x.val),clabel)
    predictors_val = as.data.frame(x.val[,predictors_name_val])
    predictand = x.val[, clabel]
    alvo <- as.factor(predictand)
    predictors_val[ncol(predictors_val)+1] = alvo
    colnames(predictors_val)[ncol(predictors_val)] = clabel
    x.val = predictors_val
  }

  
  h2o.init()
  train.hex <- as.h2o(x.train)
  test.hex <- as.h2o(x.test)
  val.hex <- as.h2o(x.val)
  
  tdeep <- h2o.deeplearning(x = 1:ncol(x.train)-1, y = ncol(x.train), training_frame = train.hex, validation_frame = val.hex,
  activation = c("RectifierWithDropout"), loss = c("CrossEntropy"),  balance_classes = TRUE, hidden = tamanho, ...)
  predictions <- h2o.predict(tdeep, test.hex)
